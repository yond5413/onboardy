...
## 15. Execution Plan: Weeks 1–2 (Solo Developer Mode)

This section converts the roadmap into a concrete first two-week delivery plan focused on **Solo Developer Mode** only.

### 15.1 Scope guardrails (must hold)
- Do not add workspace/team permissions in this phase.
- Do not block release on team-directory integrations.
- Any team-mode tasks are discovery-only and non-blocking.

### 15.2 Week 1 goals
1. Ship graph-to-chat interaction end-to-end behind a feature flag.
2. Add telemetry for graph-triggered chat usage.
3. Validate user latency and quality for node-triggered prompts.

### 15.3 Week 1 implementation backlog

#### Day 1 (Design + contracts)
- Finalize `graphContext` payload schema for chat requests.
- Define quick-action prompt templates:
  - Explain this component
  - Trace inbound/outbound flow
  - Where to debug
  - Files to read first
- Add event names for analytics (`graph_node_clicked`, `graph_prompt_sent`, `graph_chat_completed`).

#### Day 2 (Frontend wiring)
- Architecture view: node click opens action menu.
- Selecting action pre-fills chat input and auto-navigates to Chat tab.
- Add graceful fallback when graph metadata is missing.

#### Day 3 (Backend acceptance)
- Extend chat endpoint to accept optional `graphContext`.
- Persist graph context with each chat turn metadata.
- Ensure compatibility with existing non-graph chat requests.

#### Day 4 (Prompting + response structure)
- Update agent prompt builder to incorporate graph context fields when present.
- Add response-format guidance to include:
  - explanation
  - relevant files
  - related components
  - recommended next question
...
- Manual QA pass for happy-path and fallback-path.
- Capture latency and success metrics from telemetry.
- Internal demo and go/no-go decision for Week 2 rollout.

### 15.4 Week 1 acceptance criteria
- Node click to chat-prefill works in under 250ms on local/staging baseline.
- No regressions for existing manual chat entry.
- At least one graph-originated chat event path is fully observable in analytics.

### 15.5 Week 2 goals
1. Deliver Who-to-Ask Solo MVP (top 1–3 likely owners + rationale).
2. Expose ownership hints in architecture/details views.
3. Add minimal confidence scoring and stale-data safeguards.

### 15.6 Week 2 implementation backlog

#### Day 6 (Ownership extraction prototype)
- Implement git history extraction for key files/components.
- Collect author, timestamp, and touched-file counts.
- Add bot/service-account filtering heuristics.

#### Day 7 (Scoring model)
- Implement owner confidence score using:
  - contribution frequency
  - recency weighting
  - file relevance mapping
- Add stale confidence downgrade logic.

#### Day 8 (Data model + storage)
- Define ownership JSON shape per component.
- Persist ownership output with analysis artifacts.
- Add fallback state when ownership cannot be determined.

#### Day 9 (UI integration)
- Add “Likely owners” section in architecture/details surfaces.
- Show confidence badge and rationale text.
- Keep display to top 1–3 candidates (solo-mode constraint).

#### Day 10 (Validation + release prep)
- Validate ownership display quality on at least 3 sample repos.
- Add telemetry (`owner_panel_viewed`, `owner_suggestion_used`).
- Prepare release notes and Week 3 backlog handoff.

### 15.7 Week 2 acceptance criteria
- Ownership appears for a meaningful subset of components with rationale.
- No PII leakage in UI (email redaction policy respected where applicable).
- Existing analysis and chat flows remain functional.

### 15.8 Weekly metrics review cadence
- End of Week 1:
  - graph chat initiation rate
  - node-click to prompt-send conversion
  - chat depth for graph-triggered sessions
- End of Week 2:
  - % components with ownership suggestions
  - confidence distribution and stale-rate
  - ownership panel engagement

### 15.9 Risks in weeks 1–2 and mitigations
- **Graph label quality variance** → apply default prompt fallbacks and neighbor context enrichment.
- **Noisy git history (bots/drive-by commits)** → filter identities and prioritize recency-weighted signals.
- **Scope creep into team mode** → enforce solo-only acceptance criteria for both weeks.
